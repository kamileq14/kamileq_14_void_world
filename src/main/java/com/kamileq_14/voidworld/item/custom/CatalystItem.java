package com.kamileq_14.voidworld.item.custom;
...
public class CatalystItem extends Item {
    public CatalystItem() {
        super(new Properties()
                .tab(ModCreativeModeTab.VOIDWORLD_TAB)
                .stacksTo(1)
                .rarity(Rarity.RARE)
        );
    }
    ...
    public InteractionResult useOn(UseOnContext context) {
        if(context.getPlayer() != null) {
            if(context.getPlayer().level.dimension() == ModDimensions.VOIDDIM_KEY
                    || context.getPlayer().level.dimension() == Level.OVERWORLD) {
                for(Direction direction : Direction.Plane.VERTICAL) {
                    BlockPos framePos = context.getClickedPos().relative(direction);
                    if(((VoidPortalBlock) ModBlocks.VOID_PORTAL.get()).trySpawnPortal(context.getLevel(), framePos)) {
